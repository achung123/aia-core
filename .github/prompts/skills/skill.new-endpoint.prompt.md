...
description: Shared skill — scaffold a new FastAPI endpoint end-to-end (route + Pydantic schema + DB access + test) following aia-core conventions.
---
...
Do **not** use this skill for schema-only changes (update the Pydantic model in place) or for DB-only changes (see `skill.db-migration`).
...
3. **Follow `skill.tdd-cycle`.** Before writing the handler, add a `test_<feature>_api.py` under `backend/test/` that exercises the new endpoint through `TestClient`. Assert status code, response shape, and any DB side effects. Confirm Red.
...
6. **Green the test**, run the full suite (`cd backend && uv run pytest test/`), then refactor per `skill.tdd-cycle`.
7. **If the request touches new DB columns or tables** → stop and run `skill.db-migration` first.
