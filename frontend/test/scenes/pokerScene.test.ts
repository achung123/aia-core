...
import type { PokerSceneResult, PokerSceneOptions } from '../../src/../src/scenes/pokerScene.ts';
...
describe('createPokerScene', () => {
  ...
  beforeEach(async () => {
    ({ createPokerScene } = await import('../../src/scenes/pokerScene.ts'));
  });
...
describe('touch controls', () => {
  ...
  it('default camera position matches DEFAULT_OVERHEAD_POSITION from seatCamera', async () => {
    const { DEFAULT_OVERHEAD_POSITION } = await import('../../src/scenes/seatCamera.ts');
    const canvas = makeCanvas();
    const result = createPokerScene(canvas);
