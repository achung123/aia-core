...
def _get_player_name_for_seat(
    ...
    if game_player is None:
        return None
    player = db.query(Player).filter(Player.player_id == game_player.player_id).first()
...
def get_hand_status(
    ...
    for player_hand in hand.player_hands:
        latest = (
            ...
            .filter(PlayerHandAction.player_hand_id == player_hand.player_hand_id)
    ...
    for player_hand in hand.player_hands:
        ...
        total = round(
            sum(action_row.amount for action_row in all_actions if action_row.amount),
            2,
        )
    ...
    for player in game.players:
        ...
        players.append(
            PlayerStatusEntry(
                ...
                outcome_street=(player_hand.outcome_street if player_hand else None),
                last_action=(
                    last_action_by_player_hand_id.get(player_hand.player_hand_id)
                    if player_hand
                    else None
                ),
                current_chips=chips_by_player_id.get(player.player_id),
                pot_contribution=contribution_by_player_id.get(player.player_id, 0),
            )
        )
    ...
    for player_entry in body.players:
        player_entry.is_current_turn = (
            body.current_player_name is not None
            and player_entry.name == body.current_player_name
        )
...
def get_hand_equity(
    ...
    for player_hand in hand.player_hands:
        ...
        if player_hand.player_id in folded_player_ids:
            continue
        player_row = (
            db.query(Player).filter(Player.player_id == player_hand.player_id).first()
        )
        player_name = player_row.name if player_row else ''
        hole_cards = [
            _db_card_to_tuple(player_hand.card_1),
            _db_card_to_tuple(player_hand.card_2),
        ]
    ...
    if player is not None:
        ...
        if num_opponents < 1:
            return EquityResponse(equities=[])
        equity_value = calculate_player_equity(
            target_cards, num_opponents, community_cards
        )
    ...
    return EquityResponse(
        equities=[
            ...
            for (name, _, raw), equity_value in zip(
                players_with_cards, equities, strict=False
            )
        ]
    )
...
def _build_hand_response(hand: Hand, db: Session) -> HandResponse:
    ...
    player_id_to_name = {
        player_row.player_id: player_row.name for player_row in players
    }
    ...
    for player_hand in hand.player_hands:
        ...
        hand_desc = (
            describe_hand(hole_cards, community) if len(hole_cards) == 2 else None
        )
...
def edit_player_hole_cards(
    ...
    player_hand = get_player_hand_or_404(
        db, hand.hand_id, player.player_id, player_name
    )
    ...
    if state and state.phase == 'awaiting_cards':
        all_have_cards = all(
            player_hand_entry.card_1 is not None
            for player_hand_entry in hand.player_hands
        )
        if all_have_cards:
...
def update_player_result(
    ...
    if payload.outcome_street and payload.result != 'handed_back':
        ...
        for other_player_hand in other_player_hands:
            if other_player_hand.outcome_street and other_player_hand.result in (
                'won',
                'lost',
            ):
                showdown_streets.add(other_player_hand.outcome_street)
            elif (
                other_player_hand.outcome_street
                and other_player_hand.result == 'folded'
            ):
...
def record_player_action(
    ...
    if hand_state:
        ...
        for action_record in actions_data:
            ...
            if action_record['action'] in ('blind', 'call', 'bet', 'raise'):
                street_contribs[action_player_id] = street_contribs.get(
                    action_player_id, 0
    ...
    if payload.action == 'fold':
        non_folded_player_hands = [
            ...
            if player_hand_row.result != 'folded'
        ]
    ...
    if all_in_player_hands:
        ...
        for player_hand_row in hand.player_hands:
            total = sum(
                ...
                if action_record.action in ('blind', 'call', 'bet', 'raise')
                and action_record.amount
            )
            cumulative[player_hand_row.player_id] = total
        non_folded_ids = {
            player_hand_row.player_id
            for player_hand_row in hand.player_hands
            if player_hand_row.result != 'folded'
        }
        all_in_ids = {
            player_hand_row.player_id for player_hand_row in all_in_player_hands
        }
    ...
    if hand_state:
        ...
        seats_excl_all_in = [
            (seat_number, player_id)
            for seat_number, player_id in seats
            if player_id not in all_in_ids
        ]
        actions_data = get_actions_this_street(db, hand, hand_state.phase)
