...
def _det(value: str, bbox_x: float, bbox_y: float, w: float = 0.05, h: float = 0.07) -> DetectionResult:
...
class TestPositionAssignerConstructor:
    ...
    def test_custom_parameters(self):
        pa = PositionAssigner(community_y_min=0.1, community_y_max=0.5, min_community_cards=4)
...
class TestHoleCardAssignment:
    def test_community_and_hole_cards(self):
        ...
        result = pa.assign(detections, 1920, 1080)
        community = [r for r in result if r.card_position and not r.card_position.startswith("hole")]
        holes = [r for r in result if r.card_position and r.card_position.startswith("hole")]
