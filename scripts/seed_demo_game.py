    ...
    try:
        ...
        for game_num in range(NUM_GAMES):
            ...
            num_players = random.randint(
                MIN_PLAYERS, min(MAX_PLAYERS, len(ALL_PLAYERS))
            )
            game_player_names = sorted(random.sample(ALL_PLAYERS, num_players))

            game = GameSession(
                game_date=game_date,
                status='completed' if game_num < NUM_GAMES - 1 else 'active',
            )
            db.add(game)
            db.flush()
            ...
            for name in game_player_names:
                db.add(
                    GamePlayer(
                        game_id=game.game_id, player_id=player_map[name].player_id
                    )
                )
            ...
            db.flush()
            print(
                f'  Game {game.game_id}: {game_date} — {len(game_player_names)} players, {HANDS_PER_GAME} hands'
            )
